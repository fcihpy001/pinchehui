...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _chatterOrGroupArray = [NSMutableArray array];
...
#pragma -mark     cell的名字，，需要把 id转换为用户真实姓名
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [dict setValue:chatController forKey:@"chatController"];
    [dict setValue:title forKey:@"title"];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"goToChatView" object:nil userInfo:dict];
...
-(void)refreshDataSource
{
    ...
    NSLog(@"refreshData11=%@",self.dataSource);
    for (int i = 0; i<[self.dataSource count]; i++) {
        NSLog(@"refreshData%d%d%d%d",i,i,i,i);
        EMConversation *conversation = [self.dataSource objectAtIndex:i];
        if (conversation.isGroup) {
            [self.dataArray addObject:conversation.chatter];
            [self.chatterOrGroupArray addObject:@"group"];
            NSLog(@"refreshData3333");
        }else{
            [self.idArray addObject:conversation.chatter];
            [self.dataArray addObject:conversation.chatter];
            [self.chatterOrGroupArray addObject:@"chatter"];
            NSLog(@"refreshData4444");
        }
    }
    ChangeIdToName * change = [[ChangeIdToName alloc]init];
    NSString * nameStr = [change changeIdToNameWith:self.idArray];
